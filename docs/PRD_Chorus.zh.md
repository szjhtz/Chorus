...
授权由 Agent 上的一个 **5 × 3 权限矩阵**（5 个资源 × 3 种动作 = 15 个权限位）驱动。UI 提供三个命名 **预设**，每个预设展开为一个固定的位模式；同时提供 **Custom** 模式，允许在预设基础上（或完全脱离预设）自由组合权限位：

| 预设 | 有效权限 | 典型 Skill 用途 |
|------|----------|----------------|
| **Developer**（`developer_agent`） | `*:read` + `task:write`（6 位） | 执行任务、汇报工作、提交验证 |
| **PM**（`pm_agent`） | `*:read` + `idea/proposal/document/task/project:write`（10 位） | 需求分析、Elaboration、起草 Proposal、任务拆解 |
| **Admin**（`admin_agent`） | 全部 15 位（`*:read` + `*:write` + `*:admin`） | 代理人类审批：批准 Proposal、验证 Task、管理 Project |

**⚠️ `*:admin` 是人类级别权限**：覆盖 Proposal 审批、Task 验收、Idea/Document 删除等关键操作。仅在需要自动化人类审批流程时才授予。

**MCP 工具暴露完全由有效权限集驱动**：每个受控 MCP 工具声明一个必需权限位（例如 `task:write`、`proposal:admin`），只有必需位在 Agent 有效权限集中的工具才会被注册。公共工具（`chorus_get_*`、`chorus_checkin`、评论、Session）不做权限门禁。完整的工具 → 权限映射见 `src/mcp/tools/permission-map.ts`；权限模型概念参考见 [`docs/PERMISSIONS.md`](./PERMISSIONS.md)。

Custom 权限是一等公民：可以自由组合权限位（例如 Developer 预设 + `task:admin` 让 Agent 自验任务，或仅授予 `*:read` 的只读审计 Agent）。Handler 级别的运行时守卫仍然生效——例如即使有 `task:write`，也只有任务的 assignee 才能调用 `chorus_submit_for_verify`。
...
```json
{
  ...
  "ideaTracker": {
    "<project-uuid>": { "name": "...", "ideas": [/* 该 project 下最近的 in-progress ideas */] }
  },
...
- ❌ Git 集成
- ✅ ~~复杂权限（RBAC）~~ — 已实现：细粒度 Agent 权限（5 资源 × 3 动作 = 15 位权限矩阵），提供预设 + Custom 模式（v0.7.0）
- ❌ 多 PM Agent 协作
